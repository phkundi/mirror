...
class Time(Frame):
	...
	def __init__(self, parent, *args, **krwargs):
		...
		# Make Time Labels
		self.timeLabel = Label(self, font=('Helvetica', text_big), text=self.time, fg='white', bg='black')
		self.dateLabel = Label(self, font=('Helvetica', text_small), text=self.date, fg='white', bg='black')
...
class Weather(Frame):
	...
	def __init__(self, parent, *args, **kwargs):
		...
		self.tempLabel = Label(self.weather_frame, font=('Helvetica', text_xlarge), fg='white', bg='black')
	...
	def get_weather(self):
		try:
			if latitude is None and longitude is None:
				...
				# get weather
				weather_req_url = "https://api.darksky.net/forecast/%s/%s,%s?lang=%s&units=%s" % (weather_api, lat, lon, weather_lang, weather_unit)
			else:
				location = ''
				weather_req_url = "https://api.darksky.net/forecast/%s/%s,%s?lang=%s&units=%s" % (weather_api, latitude, longitude, weather_lang, weather_unit)

			req = requests.get(weather_req_url)
			weather = json.loads(req.text)

			degree_sign = u'\N{DEGREE SIGN}'
			temperature2 = '{}{}'.format(int(weather['currently']['temperature']), degree_sign)
...
class News(Frame):
	...
	def __init__(self, parent, *args, **kwargs):
		...
		self.newsLabel = Label(self, text=self.title, font=('Helvetica', text_medium), fg='white', bg='black', pady=10)
